...
% === PARAMETRI COMUNI ===
% =========================================================================
tipo_prova  = 'lungo';   % Tipologia sensore: 'lungo', 'medio' o 'corto'
L0          = 70;        % Lunghezza iniziale sensore (mm)
...
% Percorsi di base
base_el  = "C:\Users\gdira\OneDrive\Documents\Magistrale\Laboratorio\Instron\Voltaggio\Lungo\";
base_mec = "C:\Users\gdira\OneDrive\Documents\Magistrale\Laboratorio\Instron\Forza_Deformazione\Lungoisteresi.is_tcyclic_Exports\";
base_out = ".\Dati\";
...
for p = 1:num_prove
    ...
    file_el  = base_el  + sprintf("Lungoisteresi%d.txt", cpm_current);
    file_mec = base_mec + sprintf("Lungoisteresi_%d.csv", p);
    ...
    % Chiamata con la nuova firma (tipo_prova in 7a posizione, R0 in 13a)
...
% =========================================================================
% 1. Cartella di destinazione coerente: ./Grafici/isteresi_<tipo_prova>/
...
% 2. Creazione figura
fig = figure('Name', 'Hysteresis Mean and Std vs Frequency', ...
             'Color', 'w', ...
             'Units', 'pixels', ...
             'Position', [100, 100, 950, 620]);

c_bar  = [0.20, 0.45, 0.70];   % Blu tenue/elegante
c_err  = [0.15, 0.15, 0.15];   % Grigio scuro/antracite
c_mean = [0.85, 0.15, 0.15];   % Rosso acceso per marker media

% 3. Plot a barre (solo Media) con barre di errore (± Std)
x_pos = 1:num_prove;
...
title({sprintf('Mean Hysteresis and Standard Deviation - Long Sensor')}, ...
       'FontSize', 14, 'FontWeight', 'bold');

% Margine superiore per ospitare l'annotazione
...
% 6. Salvataggio automatico in formato vettoriale PDF
...
% Funzione helper locale per formattazione titolo
