% === PERCORSI DEI FILE ===
file_elettrico = "C:\Users\gdira\OneDrive\Documents\Magistrale\Laboratorio\Instron\Voltaggio\Lungo\Lungostatica6.txt"; 
file_meccanico = "C:\Users\gdira\OneDrive\Documents\Magistrale\Laboratorio\Instron\Forza_Deformazione\lungostatica.is_tens_Exports\lungostatica_6.csv"; 
file_output = 'dati_lungo_statica6.csv'; 
...
[min_volt, indice_min] = min(voltaggio_filtrato);

% 2. Quanti campioni (righe) ha la prova meccanica?
N_campioni = length(t1);

% 3. Calcolo da quale indice del voltaggio devo iniziare a "tagliare"
% affinché la fine coincida con il minimo, avendo la stessa lunghezza della meccanica
inizio_taglio = indice_min - N_campioni + 1;

% Controllo di sicurezza: verifica se hai iniziato a registrare il voltaggio 
% prima di far partire la prova meccanica (come si fa di solito)
if inizio_taglio < 1
    error('Attenzione: la registrazione del voltaggio è partita troppo tardi per coprire tutta la prova meccanica!');
end

% 4. Estraggo solo la fetta di voltaggio che mi interessa
